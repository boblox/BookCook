...
export class RecipesService implements IRecipesService {
    ...
    getRecipeRevisions(recipeId: number): Observable<RecipeRevision[]> {
        return this.http.get(`${this.apiUrl}/api/recipes/${recipeId}/history/`)
            .map((result: any[]) => {
                    return result.map(i => this.mapperService.recipeRevisionMapper.getClientModel(i));
                },
                error => console.error(error));
    }

    getRecipe(recipeId: number): Observable<Recipe> {
        return this.http.get(`${this.apiUrl}/api/recipes/${recipeId}/`)
            .map((result: any[]) => {
    ...
    deleteRecipe(recipeId: number): Observable<boolean> {
        return this.http.delete(`${this.apiUrl}/api/recipes/${recipeId}/`)
            .map((result: boolean) => {
                    return result;
                },
                error => console.error(error));
